...
function terragruntFmt {
  ...
  if [ ${fmtExitCode} -eq 0 ]; then
    ...
    echo "${BRed}fmt: error: failed to parse Terraform files${NC}"
    echo "${fmtOutput}"
    echo
    exit ${fmtExitCode}
  else
		# Exit code of !0 and !2 indicates failure.
		echo "${BRed}fmt: error: Terraform files in ${tfWorkingDir} are incorrectly formatted${NC}"
		echo "${fmtOutput}"
		echo
		echo "${BRed}fmt: error: the following files in ${tfWorkingDir} are incorrectly formatted${NC}"
